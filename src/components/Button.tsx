...
export default function Button({ onClick, label, variant = "" }: ButtonProps) {
    ...
    return (
        <button
            onClick={() => onClick?.()}
            className={`mb-3 w-full rounded border border-[#009963] px-4 py-2 font-semibold hover:cursor-pointer focus-visible:border-[#FF4D8D] focus-visible:ring-1 focus-visible:ring-[#FF4D8D] focus-visible:outline-none ${additionalStyles}`}
        >
            {label}
        </button>
    );
